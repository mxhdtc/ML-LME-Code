...
names(data1) <- c('PW','PH(V)','PH(V)/PW', 'PH', 'PH/PW', 'SA', 'IFD', 'SFD', 'G_g', 'f1', 'f2', 'f3', 'f4', 'f5', 'f6', 'f7', 'f8', 'f9', 'f10', 'f11', 'f12', 'LD1', 'LD2', 'LD3', 'LD4', 'LD5', 'LD6', 'fresh_weight', 'dry_weight', 'plant height(cm)', 'green_leaf_area')  # translate the variable name into English
...
modelmax=lmer(formula = log(fresh_weight)~log(SandBox)+(1+SandBox|frac_SandBox),data=data1)
# using LME only considering random Intercept
modelzero=lmer(formula = log(fresh_weight)~log(PH_V)+log(SA)+log(G_g)+log(SandBox)+(1|frac_SandBox),data=data1)
linearmodel = lm(formula = log(fresh_weight)~log(PH_V)+log(SA)+log(G_g)+log(SandBox),data=data1)
...
p + geom_point() + geom_smooth(method = "lm")+labs(x = 'Estimations', y = 'Sample Observations')+theme(title = element_text(size = 20), axis.title.x = element_text(size = 20), axis.title.y = element_text(size = 20))+ scale_x_continuous(limits = c(0,710),expand = c(0,0)) + scale_y_continuous(limits = c(0,710),expand = c(0,0))
...
p <- ggplot(data = data.frame(x = exp(lm_pred), y = data1$fresh_weight), mapping = aes(x = x, y = y))
p + geom_point() + geom_smooth(method = "lm")+labs(x = 'Estimations', y = 'Sample Observations')+theme(title = element_text(size = 20), axis.title.x = element_text(size = 20), axis.title.y = element_text(size = 20))+ scale_x_continuous(limits = c(0,850),expand = c(0,0)) + scale_y_continuous(limits = c(0,850),expand = c(0,0))#画出回归图像
...
df <- data.frame(x = c(0.1, 0.3, 0.5, 0.7, 0.9, 1.2, 1.5), y = c(0.2, 0.59, 1.03, 1.38, 1.82, 2.37, 3.02))
lm <- lm(y ~ x, data = df) ### 线性拟合
pre_y <- predict(lm) ### 在原来自变量上预测
...
```
```{r}
data.frame(x = lm_pred, y = lm_pred)
```
